	...
	<div class="fixed bottom-0 left-0 right-0 z-50 bg-base-200 border-t border-base-300 shadow-lg">
		<div class="max-w-3xl mx-auto px-4 py-3">
			...
			{#if audioState.radioMode}
				<!-- Radio mode: station name + play/pause + close only -->
				<div class="flex items-center justify-between gap-3">
					<div class="min-w-0">
						<p class="text-xs font-semibold text-base-content truncate">{audioState.radioStationTitle}</p>
						{#if audioState.radioStationSubtitle}
							<p class="text-[0.65rem] text-base-content/70 truncate">{audioState.radioStationSubtitle}</p>
						{/if}
					</div>
