<script lang="ts">
	...
	import type { StudyTab } from '../StudyModeModal/types';

	interface Props {
		verseKey: string;
		counts?: VerseTabCounts | null;
		onTabClick: (tab: StudyTab) => void;
	}
	...
	const tabs = $derived(
		counts === null
			? ALL_TABS.filter((t) => t.always)
			: ALL_TABS.filter((t) => {
					if (t.always) return true;
					if (t.id === 'layers') return counts.hasLayers;
					if (t.id === 'answers') return counts.hasAnswers;
					if (t.id === 'qiraat') return counts.hasQiraat;
					if (t.id === 'hadith') return counts.hasHadith;
					if (t.id === 'related-verses') return counts.hasRelatedVerses;
					return true;
			  })
	);
</script>

<div class="flex items-center overflow-x-auto border-t border-base-200 px-1 sm:px-2 scrollbar-none">
	{#each tabs as tab, i (tab.id)}
		{#if i > 0}
			<div class="h-3.5 w-px shrink-0 bg-base-300"></div>
		{/if}
		<button
			data-testid="bottom-action-tab-{tab.id}"
			class="flex shrink-0 items-center gap-1 px-1.5 py-2.5 text-[0.65rem] text-base-content/50 transition-colors hover:text-base-content sm:gap-1.5 sm:px-3 sm:text-xs [&_svg]:h-3.5 [&_svg]:w-3.5 sm:[&_svg]:h-4 sm:[&_svg]:w-4"
		...
		>
			...
			{:else if tab.id === 'lessons'}
				<svg
					...
					><path
						fill="currentColor"
						d="M5.434 13.804 10 16.484l4.566-2.68V10.91l1.534-.89v4.74l-6.1 3.58-6.1-3.58v-4.74l1.534.89zM2.888 6.548 10 10.58l7.112-4.032L10 2.786zM19.1 11.545h-1.486V7.993l-7.614 4.317-9.05-5.13V5.896l9.05-4.79 9.1 4.79z"
					/></svg
				>
			{:else if tab.id === 'reflections'}
				<svg
					...
					stroke-linejoin="round"
					><path
						d="M21 11.5a8.4 8.4 0 0 1-.9 3.8 8.5 8.5 0 0 1-7.6 4.7 8.4 8.4 0 0 1-3.8-.9L3 21l1.9-5.7a8.4 8.4 0 0 1-.9-3.8 8.5 8.5 0 0 1 4.7-7.6 8.4 8.4 0 0 1 3.8-.9h.5a8.48 8.48 0 0 1 8 8z"
					/></svg
				>
